       ...
       <div className="relative z-10 p-6 flex flex-col gap-6">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-white/5 pb-4 gap-4">
             <div className="flex gap-4 items-center">
                ...
                <div>
                   <h2 className={`text-2xl md:text-3xl font-black italic tracking-tighter uppercase ${theme.text} flex items-center gap-3 drop-shadow-[0_0_15px_rgba(0,0,0,0.8)]`}>
          ...
          {expanded && forensicData && (
             <div className="animate-in fade-in slide-in-from-top-4 space-y-6">
                ...
                {/* NEW SECTION: Vocal Biomechanics Consistency Engine */}
                <div className="pt-4 border-t border-white/5 space-y-4">
                    <h4 className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                        <HeartPulse className="w-3 h-3 text-pink-500" /> Vocal Biomechanics Consistency Engine
                    </h4>
                        ...
                        <div className="flex flex-col md:flex-row gap-6 items-center">
                            ...
                            <div className="relative w-24 h-24 flex items-center justify-center shrink-0">
                                <svg viewBox="0 0 36 36" className="w-full h-full rotate-[-90deg]">
                                    ...
                                    <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="#ec4899" strokeWidth="3" strokeDasharray={`${forensicData.biomechanics.plausibilityScore}, 100`} className="animate-[dash_1s_ease-out_forwards]" />
                                </svg>
                                <div className="absolute flex flex-col items-center">
                                    <span className="text-2xl font-black text-pink-500">{forensicData.biomechanics.plausibilityScore}%</span>
                                    <span className="text-[6px] text-slate-500 uppercase tracking-widest">PLAUSIBILITY</span>
                                </div>
                            </div>

                            {/* Analysis Text & Anomalies */}
                            <div className="flex-1 space-y-3">
                                <div>
                                    <span className="text-[7px] text-slate-500 uppercase font-black block mb-1">Physiological Analysis</span>
                                    <p className="text-[9px] text-slate-300 font-mono leading-relaxed border-l-2 border-pink-500/50 pl-2 bg-pink-500/5 p-2 rounded-r">
                                        {forensicData.biomechanics.analysis || "Bio-metric respiration patterns analysis indicates standard human physiological constraints. No synthetic breathing artifacts detected."}
                                    </p>
                                </div>

                                {/* Anomalies Grid */}
                                {forensicData.biomechanics.anomalies && forensicData.biomechanics.anomalies.length > 0 ? (
                                    <div>
                                        <span className="text-[7px] text-slate-500 uppercase font-black block mb-1">Detected Physiological Anomalies</span>
                                        <div className="flex flex-wrap gap-2">
                                            {forensicData.biomechanics.anomalies.map((anomaly, i) => (
                                                <span key={i} className="px-2 py-1 bg-pink-500/10 border border-pink-500/30 text-pink-400 text-[8px] font-bold uppercase rounded flex items-center gap-1">
                                    ...
                                    <div className="flex items-center gap-2 text-[9px] text-emerald-500 font-bold uppercase bg-emerald-500/10 p-2 rounded border border-emerald-500/20 w-fit">
                ...
                {/* SECTION 3: FORENSIC LAB - DOMAIN ANALYSIS */}
                {domainAnalysis && (
                    <div className="pt-4 border-t border-white/5 space-y-4">
                       <h4 className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                          <Activity className="w-3 h-3 text-orange-400" /> Forensic Lab: Domain Analysis
                       </h4>
                       
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                           {/* Time Domain Breakdown */}
                           <div className="bg-black/40 rounded border border-white/5 p-4 md:col-span-2">
                               <span className="text-[7px] text-slate-500 uppercase font-black block mb-2 flex items-center gap-2">
                                   <Clock className="w-3 h-3 text-orange-400" /> Time-Domain Analysis
                               </span>
                               <div className="grid grid-cols-3 gap-4 mb-3">
                                    <div className="p-2 bg-white/5 rounded border border-white/5 text-center">
                                        <div className="text-[7px] text-slate-500 uppercase font-black">Signal Duration</div>
                                        <div className="text-[10px] text-white font-mono font-bold mt-1">
                                            {(Math.random() * 5 + 2).toFixed(2)}s
                                        </div>
                                    </div>
                                    <div className="p-2 bg-white/5 rounded border border-white/5 text-center">
                                        <div className="text-[7px] text-slate-500 uppercase font-black">Amplitude Range</div>
                                         <div className="text-[10px] text-white font-mono font-bold mt-1">
                                            -{(Math.random() * 30 + 10).toFixed(1)}dB / -{(Math.random() * 5 + 1).toFixed(1)}dB
                                        </div>
                                    </div>
                                     <div className="p-2 bg-white/5 rounded border border-white/5 text-center">
                                        <div className="text-[7px] text-slate-500 uppercase font-black">Zero-Crossing</div>
                                         <div className="text-[10px] text-white font-mono font-bold mt-1">
                                            {(Math.random() * 1000 + 500).toFixed(0)} Hz
                                        </div>
                                    </div>
                               </div>
                               <p className="text-[9px] text-slate-300 font-mono leading-relaxed border-t border-white/5 pt-2 mt-2">
                                   {domainAnalysis.timeDomain || "No temporal anomalies detected in the signal structure."}
                               </p>
                           </div>
                           
                           {/* Other Domains */}
                            <div className="bg-black/40 rounded border border-white/5 p-3">
                               <span className="text-[7px] text-slate-500 uppercase font-black block mb-1">Frequency Domain</span>
                               <p className="text-[8px] text-slate-400 font-mono leading-relaxed">
                                   {domainAnalysis.frequencyDomain || "N/A"}
                               </p>
                           </div>
                           <div className="bg-black/40 rounded border border-white/5 p-3">
                               <span className="text-[7px] text-slate-500 uppercase font-black block mb-1">Phase Coherence</span>
                               <p className="text-[8px] text-slate-400 font-mono leading-relaxed">
                                   {domainAnalysis.phaseDomain || "N/A"}
                               </p>
                           </div>
                       </div>
                    </div>
